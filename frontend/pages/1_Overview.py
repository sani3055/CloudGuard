...
from utils import inject_css, load_data, render_metric_card
import boto3
...
if df.empty:
    st.info("No security events detected yet in CloudGuard-ThreatEvents.")
    st.stop()
...
st.markdown('<div class="soc-header">Risk Intelligence</div>', unsafe_allow_html=True)
...
fig_time = px.scatter(
    ...
    hover_data=["eventName", "userIdentitytype", "severity", "awsRegion"],
